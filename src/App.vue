    ...
    <div v-if="showContactModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
          ...
          <div class="bg-gradient-to-r from-purple-50 to-pink-50 px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                <!-- Back Button (only show when viewing email) -->
                <button 
                  v-if="showEmailDraft"
                ...
                <div>
                  <h3 class="text-lg font-semibold text-gray-900">
                    {{ showEmailDraft ? 'Draft Email' : isGeneratingEmail ? 'Generating Email...' : 'Find Contacts' }}
                  </h3>
                  <p class="text-sm text-gray-600">
                    {{ showEmailDraft && selectedContact ? 
                        `${selectedContact.firstName} ${selectedContact.lastName} • ${selectedJob?.company}` : 
                        selectedJob?.company 
                    }}
                  </p>
                </div>
          ...
          <div class="p-6 max-h-[70vh] overflow-y-auto">
            ...
            <div v-if="isGeneratingEmail" class="text-center py-12">
              <div class="inline-flex items-center space-x-3 mb-4">
                <div class="animate-spin rounded-full h-10 w-10 border-b-2 border-purple-500"></div>
                <span class="text-lg font-medium text-gray-700">Generating personalized email...</span>
              </div>
              <p class="text-sm text-gray-500 mb-6">This may take 10-15 seconds...</p>
              
              <!-- Contact info being processed -->
                ...
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 bg-gradient-to-r from-purple-100 to-pink-100 rounded-full flex items-center justify-center">
                    <span class="text-sm font-semibold text-purple-700">
                      {{ (selectedContact?.firstName || selectedContact?.email || '').charAt(0).toUpperCase() }}
                    </span>
                  </div>
                  <div class="text-left">
                    <p class="font-medium text-gray-900">{{ selectedContact?.firstName }} {{ selectedContact?.lastName }}</p>
                    <p class="text-sm text-gray-600">{{ selectedContact?.position }}</p>
                    <p class="text-xs text-gray-500">{{ selectedContact?.email }}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <!-- Email Draft View -->
            <div v-else-if="showEmailDraft && generatedEmail" class="">
              <div class="bg-gray-50 rounded-lg p-6 mb-6">
                <pre class="whitespace-pre-wrap text-sm text-gray-800 leading-relaxed font-sans">{{ generatedEmail }}</pre>
              </div>
              
              <div class="flex justify-end gap-3">
                <button 
                  @click="copyToClipboard(generatedEmail)"
                  class="px-6 py-2 text-sm font-medium text-white bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 rounded-lg transition-all"
                >
                  Copy Email
                </button>
              </div>
            ...
            <div v-else-if="emailError" class="text-center py-8">
              ...
              <p class="text-gray-600 text-sm mb-4">{{ emailError }}</p>
              <button 
                @click="goBackToContacts"
                class="px-4 py-2 text-sm font-medium text-purple-600 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors"
              >
                Back to Contacts
              </button>
            </div>
            
            <!-- Contact Discovery Loading State -->
            <div v-else-if="isLoadingContacts" class="text-center py-8">
          ...
          <!-- Modal Footer (only show for contact list) -->
          <div v-if="!showEmailDraft && !isGeneratingEmail && !emailError && contacts.length > 0" class="bg-gray-50 px-6 py-4 border-t border-gray-200">
